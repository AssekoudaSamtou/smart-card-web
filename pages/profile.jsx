...
export const LinkedInIcon = ({ className, sx }) => (
	<svg className={ className } sx={ sx } width="24" height="24" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export const TwitterIcon = ({ className }, sx) => (
	<svg className={ className } sx={ sx } width="24" height="24" viewBox="0 0 256 209" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export const InstagramIcon = ({ className, sx }) => (
	<svg className={ className } sx={ sx } width="24" height="24" viewBox="0 0 96 96" fill="none" xmlns="http://www.w3.org/2000/svg">
		...
		<div sx={{width: '60vw', margin: 'auto'}}>
			...
			<form>
				...
				<div sx={{display: 'flex', justifyContent: 'end'}}>
					<Button color="neutral" style={{margin: '0 1rem 0 0'}}>Annuler</Button>
					<Button color="primary">Enrégistrer</Button>
				</div>
...
const SocialNetworkSetupForm = ({ socialNetwork, onSubmit, isSubmitting }) => {
...
const [apiKey, setApiKey] = useState("")
	...
	return (
		<form onSubmit={ event => {
			...
			onSubmit && onSubmit({...socialNetwork, username, apiKey})
		}}>
			<div sx={{display: 'flex', justifyContent: 'space-between'}}>
				<TextInput style={{flexBasis: '45%'}} name="phone" value={ username } setValue={ setUsername } label={`Pseudo ${ socialNetwork.name }`} type="text"/>
				<TextInput style={{flexBasis: '45%'}} name="apikey" value={ apiKey } setValue={ setApiKey } label="API Key" type="text"/>
			</div>
			
			<div sx={{display: 'flex', justifyContent: 'end'}}>
				<div>
					<Button disabled={ isSubmitting } color="dark">
						{ isSubmitting ? (<Loading width={12} height={12} src={Loader}/>) : "Enrégistrer" }
					</Button>
				</div>
			</div>
		</form>
...
const defaultSocialNetworks = [
	{id: uuidv4(), name: 'twitter', link: 'https://twitter.com', Icon: React.createElement(TwitterIcon)},
	{id: uuidv4(), name: 'linkedin', link: 'https://linkedin.com', Icon: React.createElement(LinkedInIcon)},
	{id: uuidv4(), name: 'facebook', link: 'https://facebook.com', Icon: React.createElement(TwitterIcon)},
	{id: uuidv4(), name: 'instagram', link: 'https://instagram.com', Icon: React.createElement(InstagramIcon)},
]
const SocialNetWorkTab = () => {
	const [socialNetworks, setSocialNetworks] = useState(defaultSocialNetworks)
	const [showSetupForm, setShowSetupForm] = useState(false)
	const [isSubmitting, setIsSubmitting] = useState(false)
	const [socialNetworkToSetup, setSocialNetworkToSetup] = useState(undefined)
	...
	const socialNetworkSetupClickHandler = (socialNetwork) => {
		setSocialNetworkToSetup(socialNetwork)
		setShowSetupForm(true)
	}
	
	const setupFormSubmitHandler = (data) => {
		setIsSubmitting(true)
		setTimeout(() => {
			setSocialNetworks(prevState => {
				prevState[prevState.findIndex(item => item.id === data.id)]  = data
				return prevState
			})
			setShowSetupForm(false)
			setIsSubmitting(false)
	...
	return (
		<div sx={{display: 'flex'}}>
			...
			{
				showSetupForm && socialNetworkToSetup ? (
					<Modal handleClose={ modalCloseHandler }>
						<div sx={{width: '600px'}}>
							{ socialNetworkToSetup.Icon }
							<SocialNetworkSetupForm isSubmitting={ isSubmitting } onSubmit={ setupFormSubmitHandler } socialNetwork={ socialNetworkToSetup }/>
						</div>
					</Modal>
				) : ''
			}
			...
			<div sx={{
				...
				</div>
				{
					socialNetworks.map((item, i) => (
						<div key={ i } className='item' sx={{display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #E2E8F0', p: '1rem 0'}}>
							<div sx={{display: 'flex', alignItems: 'center'}}>
								<span sx={{
									p: '10px',
									display: 'flex',
							...
							<span sx={{alignSelf: 'center', fontWeight: '300', fontSize: '.7vmax'}}>
								{ item.username && item.apiKey ? '@' + item.username : 'Non configuré' }
							</span>
							<Button color="secondary" size="small" style={{ marginLeft: '1rem', alignSelf: 'center'}} onClick={ () => socialNetworkSetupClickHandler(item) }>Configurer</Button>
						</div>
					))
				}
			</div>
